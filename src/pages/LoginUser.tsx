                    ...
                    if (error) {
                      if (error.message.includes("Invalid login credentials") || error.message.includes("Email not confirmed")) {
                        const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
                          email: demoEmail, password: demoPassword, options: { data: { full_name: 'Demo User' } }
                        });
                        if (signUpError) throw signUpError;
                        if (!signUpData.user) throw new Error("Failed to auto-create demo user");
                        await supabase.from('profiles').upsert({
                          id: signUpData.user.id, full_name: 'Demo User', phone: '8888888888', email: demoEmail,
                          city: 'Demo City', state: 'Demo State', country: 'Demo Country',
                          security_question: 'Demo Question', security_answer: 'demo answer', updated_at: new Date().toISOString()
                        });
                        await supabase.from('user_roles').insert({ user_id: signUpData.user.id, role: 'user' });
                      ...
                      if (roleError || !roleData || roleData.role !== 'user') {
                        if (!roleData && data.user.email === demoEmail) {
                          await supabase.from('user_roles').insert({ user_id: data.user.id, role: 'user' });
                        } else {
                          await supabase.auth.signOut();
                          throw new Error('Please use owner login or role is missing');
                        }
                      }
                    }
                    toast({ title: "Demo Login successful!", description: "Welcome back to BookNex", duration: 1000 });
                    navigate('/dashboard/user');
                  } catch (error) {
                    console.error("Demo login error:", error);
                    const message = error instanceof Error ? error.message : "Invalid credentials";
                    toast({ title: "Login failed", description: message, variant: "destructive", duration: 1000 });
